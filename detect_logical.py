...
from utils.plots import plot_one_box2, plot_one_box2_ccy
...
class UI_Logic_Window(QtWidgets.QMainWindow):
    ...
    def open_camera_settings(self):
        ...
        # Create and add UI elements to camera_dialog
        label1 = QLabel("Camera Resolution:")
        edit1 = QLineEdit("1280x720")
        label2 = QLabel("Frame Rate:")
        edit2 = QLineEdit("30")
        zoom_label = QLabel("Zoom Level:")
        zoom_slider = QSlider(Qt.Horizontal)
        zoom_slider.setRange(1, 5)
        ...
        layout.addWidget(zoom_slider, 2, 1)
        layout.addWidget(move_up_button, 3, 1)
        layout.addWidget(move_down_button, 5, 1)
        layout.addWidget(move_left_button, 4, 0)
        layout.addWidget(move_right_button, 4, 2)

        button_layout = QHBoxLayout()
        button_layout.addWidget(ok_button)
        button_layout.addWidget(cancel_button)

        main_layout = QVBoxLayout(camera_dialog)
        main_layout.addLayout(layout)
        main_layout.addLayout(button_layout)

        # Center the dialog on the screen
        qr = camera_dialog.frameGeometry()
        cp = QtWidgets.QDesktopWidget().availableGeometry().center()
        qr.moveCenter(cp)
        camera_dialog.move(qr.topLeft())
    ...
    def init_slots(self):
        ...
        self.ui.pushButton_vide0_ccy.clicked.connect(self.button_camera_setting)
        self.ui.pushButton_fenxi.clicked.connect(self.button_fenxi)
    ...
    def detect(self, name_list, img):
        ...
        with torch.no_grad():
            ...
            for i, det in enumerate(pred):
                if det is not None and len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        single_info,leg,bigleg,sex = plot_one_box2_ccy(leg, bigleg, sex, xyxy, showimg, label=label, color=self.colors[int(cls)], line_thickness=2)
                        # print(single_info)
            ...
            # 将leg,bigleg,sex 追加到txt文件中
            #定义 残缺螃蟹度
            class_ok = ""
            if leg == 0 and bigleg == 0 and weight < 300:
                class_ok =  "G"
            elif leg == 0 and bigleg == 0 and weight >= 300 and weight <= 500:
                class_ok = "F"
            elif leg == 0 and bigleg == 0 and weight > 500:
                class_ok = "E"
            elif 0<leg<8 and 0<bigleg<2 and weight <= 500:
                class_ok = "D"
            elif 0<leg<8 and 0<bigleg<2 and weight > 500:
                class_ok = "C"
            elif leg <8 and bigleg==2 and weight <= 500:
                class_ok = "B"
            elif leg <8 and bigleg==2 and weight <= 500:
                class_ok = "A"
            elif leg <8 and bigleg==2 and weight > 500:
            ...
            #将leg,bigleg,sex,weight 和 class_ok 加到txt文件中
            #打开文件
            print(str(leg)+","+str(bigleg)+","+sex+","+str(weight)+","+class_ok)
            info_show = info_show + ", 残缺螃蟹度：" + class_ok + ", 小腿:" + str(leg)+", 钳子:"+str(bigleg)+", 公母："+sex + ", 重量："+str(weight) + "\n"
            with open("data/result.txt", "a") as f:
                f.write(str(leg)+","+str(bigleg)+","+sex+","+str(weight)+","+class_ok+"\n")
            # print(info_show)
    ...
    def show_video_frame(self):
        ...
        if img is not None:
            ...
            #将检测结果写入txt文件
            # if len(name_list) > 0:
            #     with open(self.output_folder + 'video_output/' + 'result.txt', 'a') as f:
            #         f.write(info_show + '\n')
    ...
    def button_camera_setting(self):
        print("Open camera setting window!")
        self.camera_setting_dialog = self.open_camera_settings()
        self.camera_setting_dialog.show()
    def button_fenxi(self):
        data = pd.read_csv('./data/result.txt', header=None, names=['leg', 'bigleg', 'gender', 'weight', 'quality'])

        # 按照公母和品质分组，并计算数量
        grouped_data = data.groupby(['gender', 'quality']).size().reset_index(name='count')
        ...
        width = 0.35
        x = [q for q in qualities]

        male_counts = []
        female_counts = []

        for q in qualities:
            male_count = grouped_data[(grouped_data['quality'] == q) & (grouped_data['gender'] == 'male')][
                'count'].tolist()
            male_counts.append(male_count[0] if male_count else 0)

            female_count = grouped_data[(grouped_data['quality'] == q) & (grouped_data['gender'] == 'female')][
                'count'].tolist()
            female_counts.append(female_count[0] if female_count else 0)

        ax.bar(x, male_counts, width, label='male')
        ax.bar(x, female_counts, width, bottom=male_counts, label='female')
        ax.set_xlabel('Quality')
        ax.set_ylabel('Count')
        ax.set_title('Gender Counts by Quality')
        ax.legend()

        # plt.show()

        # 以下是保存图像文件和加载图像文件的部分
        # plt.savefig('./data/average_weight.png')
        # img = cv2.imread('./data/average_weight.png')
        # img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        # img = cv2.resize(img, (640, 480), interpolation=cv2.INTER_AREA)

        # 将图像转换为Qt图像并将其显示在Qt界面中
        fig.canvas.draw()
        buf = fig.canvas.buffer_rgba()
        img = np.asarray(buf)
        img = cv2.cvtColor(img, cv2.COLOR_RGBA2RGB)
        img = cv2.resize(img, (640, 480), interpolation=cv2.INTER_AREA)
        QtImg = QtGui.QImage(img.data, img.shape[1], img.shape[0], QtGui.QImage.Format_RGB888)
        ...
        # # width = 0.35
        # # x = [q for q in qualities]
        # # male_counts = grouped_data[grouped_data['gender'] == 'male']['count'].tolist()
        # # female_counts = grouped_data[grouped_data['gender'] == 'Female']['count'].tolist()
        # # ax.bar(x, male_counts, width, label='male')
        # # ax.bar(x, female_counts, width, bottom=male_counts, label='Female')
        # # ax.set_xlabel('Quality')
        # # ax.set_ylabel('Count')
    ...
    def open_fenxi(self):
        ...
        # # 按照公母和品质分组，并计算平均重量和箱线图数据
        # grouped_data = data.groupby(['gender', 'quality'])
        # average_weight = grouped_data.agg({'weight': 'mean'})
        # boxplot_data = [group['weight'].tolist() for _, group in grouped_data]
        # 
        # # 绘制平均重量柱状图
        # average_weight_df = pd.DataFrame(average_weight)
        # average_weight_df.plot(kind='bar')
        # plt.title('Average Weight by Gender and Quality')
        # plt.xlabel('Gender and Quality')
        ...
        # 
        # # 绘制箱线图
        # plt.boxplot(boxplot_data, labels=grouped_data.groups.keys())
        # plt.title('Weight Distribution by Gender and Quality')
        # plt.xlabel('Gender and Quality')
        # plt.ylabel('Weight')
        # plt.show()
        # 
